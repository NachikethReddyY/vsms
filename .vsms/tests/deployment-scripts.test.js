...
test("package commands separate deployment operations and guard unsafe database changes", () => {
  ...
  assert.equal(scripts["deploy:migrate"], "node scripts/deploy/preflight.js --migration && prisma migrate deploy && prisma migrate status");
});
...
test("release verification accepts complete evidence and rejects mutable images", () => {
  ...
  const manifest = {
    ...
    infrastructure: { recoverySnapshotId: "staging-vsms-pre-abc123" },
    outcome: "succeeded",
  };
  manifest.evidence.recoveryCheckpoint = "verified";
...
test("release verification requires a reason for rollback evidence", () => {
  ...
  const manifest = {
    ...
    infrastructure: { recoverySnapshotId: "production-vsms-pre-abc123" },
    outcome: "rolled_back",
  };
  manifest.evidence.recoveryCheckpoint = "verified";
  fs.writeFileSync(manifestPath, JSON.stringify(manifest));
