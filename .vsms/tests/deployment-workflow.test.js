...
test("pull requests run quality checks without receiving cloud release access", () => {
  assert.match(release, /pull_request:\n\s+branches: \[main\]/);
  assert.match(release, /build:\n\s+if: github\.event_name == 'push'/);
  assert.match(release, /github\.event\.pull_request\.base\.sha \|\| github\.event\.before/);
  assert.match(release, /pnpm --dir react-user-dashboard audit --prod --audit-level high/);
...
test("environment deployment migrates before promotion and publishes frontend index last", () => {
  const checkpoint = environment.indexOf("Create and verify pre-migration recovery checkpoint");
  ...
  assert.ok(checkpoint > 0 && migration > checkpoint && promotion > migration);
  assert.match(environment, /aws rds create-db-snapshot/);
  assert.match(environment, /aws rds wait db-snapshot-completed/);
...
test("release manifest helper records the audit evidence without secret values", () => {
  ...
  const result = spawnSync(process.execPath, [path.join(root, "scripts/deploy/write-release-manifest.cjs"), outputs, manifest], {
    ...
    env: {
      ...
      RECOVERY_SNAPSHOT_ID: "staging-vsms-pre-bbbbbbbbbbbb-1",
  ...
  assert.equal(JSON.parse(body).evidence.recoveryCheckpoint, "verified");
});
...
test("OIDC roles bind production to its protected environment and keep duties separate", () => {
  ...
  assert.match(oidc, /ProductionDeploymentRole:[\s\S]*?rds:CreateDBSnapshot/);
