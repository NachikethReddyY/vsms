...
-- Registration stored procedures and supporting index.
--
-- Deployment source of truth: backend/prisma/migrations.
-- This file is a current, submission-ready export of the database functions
-- used by the registration workflow. Run it only against the matching schema.

CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Supports FIFO selection of the next participant to promote from the waitlist.
CREATE INDEX IF NOT EXISTS "event_registrations_waitlist_order_idx"
  ON public.event_registrations (event_id, created_at, registration_id)
...
BEGIN
  -- Lock the event row so concurrent requests cannot oversubscribe capacity.
  SELECT * INTO v_event
  ...
  IF FOUND THEN
    IF v_registration.participant_id <> p_participant_id
      OR v_registration.event_id <> p_event_id THEN
  ...
  IF v_event.status NOT IN ('PUBLISHED', 'IN_PROGRESS') THEN
    RAISE EXCEPTION 'REGISTRATION_EVENT_NOT_OPEN';
  END IF;
  IF NOT EXISTS (
    SELECT 1
  ...
  IF NOT EXISTS (
    ...
    WHERE contact.participant_id = p_participant_id
  ...
  IF EXISTS (
    SELECT 1
    FROM public.event_registrations AS registration
    WHERE registration.participant_id = p_participant_id
...
BEGIN
  ...
  IF v_registration.registration_status = 'SIGNED_UP' THEN
    ...
    IF FOUND THEN
      UPDATE public.event_registrations
      SET registration_status = 'SIGNED_UP', updated_at = NOW()
...
CREATE OR REPLACE FUNCTION public.get_event_registration_summary(p_event_id UUID)
RETURNS TABLE (
  event_id UUID,
  capacity INTEGER,
  signed_up_count BIGINT,
...
REVOKE ALL ON FUNCTION public.get_event_registration_summary(UUID) FROM PUBLIC;