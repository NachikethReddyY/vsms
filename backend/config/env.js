...
const corsOrigins = z.string().refine((value) => {
...
}, "must contain valid CORS origins");
...
const schema = z.object({
    ...
    JWT_ISSUER:
        z.string().min(1).default("vsms-api"),


    JWT_AUDIENCE:
        z.string().min(1).default("vsms-dashboard"),
    ...
    TLS_KEY_PATH:
        z.string()
        .default("../react-user-dashboard/certs/localhost-key.pem"),


    TLS_CERT_PATH:
        z.string()
        .default("../react-user-dashboard/certs/localhost.pem"),
    ...
    ONEMAP_BASE_URL:
        z.string()
        .url()
        .default("https://www.onemap.gov.sg"),


    ONEMAP_API_EMAIL:
        optionalEnv(z.string().email()),


    ONEMAP_API_PASSWORD:
        optionalEnv(z.string().min(1)),


    COGNITO_DOMAIN:
        optionalEnv(httpsUrl),
...
if (
    new URL(values.ONEMAP_BASE_URL).origin !==
    "https://www.onemap.gov.sg"
) {

    throw new Error(
        "ONEMAP_BASE_URL must use the official https://www.onemap.gov.sg origin"
    );
...
if (
    Boolean(values.ONEMAP_API_EMAIL) !==
    Boolean(values.ONEMAP_API_PASSWORD)
) {

    throw new Error(
        "ONEMAP_API_EMAIL and ONEMAP_API_PASSWORD must be configured together"
    );
