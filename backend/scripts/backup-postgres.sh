...
if ! pg_dump --dbname="$DATABASE_URL" --format=custom --no-owner --no-privileges --file="$backup_file"; then
  rm -f -- "$backup_file" "$manifest_file" "$schema_file" "$checksum_file"
...
for table_name in events participants event_registrations queue_entries screening_results sync_actions audit_logs; do
  ...
  [ "$exists" = "t" ] || { rm -f -- "$backup_file" "$manifest_file" "$schema_file" "$checksum_file"; fail "Required table is missing: $table_name"; }
...
printf 'Backup created: %s\nRows manifest: %s\nSchema manifest: %s\nChecksum: %s\nElapsed seconds: %s\n' "$backup_file" "$manifest_file" "$schema_file" "$checksum_file" "$elapsed"
