...
| `DELETE /api/v1/events/{id}` | `DELETE /api/v1/events/{eventId}` (`deleteTerminalEvent`). | Authenticated approved account plus administrator role; service permits terminal-event deletion only. | Exact path/method; stricter terminal-state rule. |
