...
type QrPass = { qrImage: string; expiresAt: string };
type QrResponse = { success: boolean; message?: string; data: QrPass };
...
export default function QRCodePage() {
  ...
  const generate = async (event: FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await apiClient.post<QrResponse>(`/qr/generate/${id}`);
      setPass(data.data);
    ...
    <div className="qr-layout">
      ...
      <section className="qr-preview" aria-labelledby="qr-preview-title" aria-live="polite">
        {pass ? <>
          ...
          <button className="secondary" type="button" onClick={() => { setPass(null); setRegistrationId(''); }}>Generate another pass</button>
        </> : <div className="qr-idle"><QrCodeIcon /><h2 id="qr-preview-title">No pass generated</h2><p>Enter a verified registration UUID to create a time-limited QR image.</p></div>}
      </section>
    </div>
  </div>;
}
