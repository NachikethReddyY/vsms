...
vi.mock('./stage4Api', async () => {
  ...
  return {
    ...
    getAdminAccount: vi.fn(async () => ({ account: { id: 'user-1', fullName: 'Asha Rao', email: 'asha@example.test', decisions: [], providerOperations: [], memberships: [] } })),
...
describe('Stage 4 rendered route behavior', () => {
  ...
  it('covers admin filters, required reasons, pagination metadata, keyboard selection, and errors', async () => {
    ...
    expect(calls.find(c => c.name === 'decideAccount')?.args).toEqual(['user-1', 'reject', 'Not eligible']);
  });
