...
function ParticipantForm({
  ...
  return (
    <form className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 md:grid-cols-2" noValidate onSubmit={onSubmit}>
      ...
      <Field label="Gender">
        <select className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" value={form.gender} onChange={(event) => update("gender", event.target.value)}>
      ...
      <Field label="Participant status">
        <select className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" value={form.status} onChange={(event) => update("status", event.target.value)}>
      ...
      <Field label="Accessibility notes">
        <textarea className="min-h-24 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" maxLength={1000} value={form.accessibilityNotes} onChange={(event) => update("accessibilityNotes", event.target.value)} />
      </Field>
  ...
  return (
    <AppShell title="Start registration">
      ...
      <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        {event ? <p className="text-sm text-slate-600">{new Date(event.eventDate).toLocaleDateString()} · {event.status}</p> : null}
        <div className="flex flex-wrap gap-3">
          <Link className="rounded-xl bg-slate-900 px-4 py-2 text-sm text-white" to={`/participants/search?eventId=${eventId}`}>Search existing participant</Link>
          <Link className="rounded-xl border border-slate-300 px-4 py-2 text-sm" to={`/participants/new?eventId=${eventId}`}>Create new participant</Link>
          <Link className="rounded-xl border border-slate-300 px-4 py-2 text-sm" to={`/events/${eventId}/registrations`}>View registrations</Link>
        </div>
      </div>
    </AppShell>
  );
  ...
  return (
    <AppShell title="Participant search">
      <div className="grid gap-6 lg:grid-cols-[360px,1fr]">
        <form className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5" onSubmit={(event) => { event.preventDefault(); void search(1); }}>
          <h2 className="text-lg font-semibold">Search before creating</h2>
          <FormErrorSummary error={error} />
          {Object.entries(criteria).map(([field, value]) => (
            <Field key={field} label={field === "participantReference" ? "Reference number" : field.replace(/([A-Z])/g, " $1")}>
          ...
          <Link className="block text-sm text-slate-700 underline" to={`/participants/new${eventId ? `?eventId=${eventId}` : ""}`}>Create new participant</Link>
        </form>
        <section className="space-y-4">
          {searched && participants.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white p-5">No matching participant found.</div> : null}
          {participants.map((participant) => (
            <article key={participant.id} className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {eventId ? <Link className="rounded-xl bg-slate-900 px-4 py-2 text-sm text-white" to={`/events/${eventId}/participants/${participant.id}/consent`}>Use participant</Link> : null}
...
export function ParticipantCreatePage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      navigate(eventId ? `/participants/${id}/emergency-contacts?eventId=${eventId}` : `/participants/${id}`);
    } catch (requestError: unknown) {
  ...
  return <AppShell title="Create participant"><ParticipantForm form={form} setForm={setForm} onSubmit={submit} submitLabel="Save and continue" submitting={submitting} feedback={error} onFieldChange={() => setError(null)} /></AppShell>;
}

export function ParticipantDetailPage() {
  const { participantId = "" } = useParams();
  const [participant, setParticipant] = useState<Participant | null>(null);
  useEffect(() => { void apiClient.get(`/participants/${participantId}`).then((response) => setParticipant(response.data.participant)); }, [participantId]);
  ...
  return (
    <AppShell title="Participant profile">
      <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <p className="text-sm text-slate-600">{participant.participantReference} · {participant.status} · {participant.contactNumber}</p>
        <div className="flex flex-wrap gap-3">
          <Link className="rounded-xl border border-slate-300 px-4 py-2 text-sm" to={`/participants/${participantId}/edit`}>Edit participant</Link>
          <Link className="rounded-xl border border-slate-300 px-4 py-2 text-sm" to={`/participants/${participantId}/emergency-contacts`}>Emergency contacts</Link>
          <Link className="rounded-xl border border-slate-300 px-4 py-2 text-sm" to={`/participants/${participantId}/consents`}>Consent history</Link>
          <Link className="rounded-xl border border-slate-300 px-4 py-2 text-sm" to={`/participants/${participantId}/history`}>Registration history</Link>
        </div>
      </div>
    </AppShell>
  );
...
export function ParticipantEditPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      navigate(eventId ? `/participants/${participantId}/emergency-contacts?eventId=${eventId}` : `/participants/${participantId}`);
    } catch (requestError: unknown) {
  ...
  return <AppShell title="Edit participant"><ParticipantForm form={form} setForm={setEditForm} onSubmit={submit} submitLabel="Save changes" submitting={submitting} feedback={error} onFieldChange={() => setError(null)} /></AppShell>;
}
        ...
        <section className="space-y-3">
          {contacts.map((contact) => (
            <article key={contact.id} className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <p className="text-sm text-slate-600">{contact.relationship} · {contact.phoneNumber} · {contact.status}</p>
              <button className="mt-2 text-sm underline" type="button" onClick={() => edit(contact)}>Edit</button>
            </article>
          ))}
          {eventId ? <PrimaryButton disabled={!contacts.some((contact) => contact.status === "ACTIVE")} type="button" onClick={() => navigate(`/events/${eventId}/participants/${participantId}/consent`)}>Continue to consent</PrimaryButton> : null}
        </section>
...
export function ConsentPage() {
  ...
  const [submitting, setSubmitting] = useState(false);
  useEffect(() => {
    apiClient.get("/consent-forms/active")
      .then((response) => setConsentForm(response.data.consentForm))
      .catch((requestError: unknown) => setError(getApiError(requestError, "Unable to load consent form.")));
  }, []);
  ...
  return (
    <AppShell title="Participant consent">
      <FormErrorSummary error={error} />
      {!consentForm ? <LoadingState label="Loading active consent form…" /> : (
        <form className="grid gap-6 lg:grid-cols-[1.15fr,0.85fr]" onSubmit={submit}>
          ...
          <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <PrimaryButton disabled={submitting} type="submit">{submitting ? "Recording…" : form.consentStatus === "ACCEPTED" ? "Accept and continue" : "Record decline"}</PrimaryButton>
          </section>
        </form>
      )}
    </AppShell>
  ...
  return (
    <AppShell title="Registration review">
      ...
      <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div><span className="text-sm text-slate-500">Consent</span><p>{review.latestConsent ? `${review.latestConsent.consentStatus} · Version ${review.latestConsent.consentFormVersion.versionNumber}` : "Missing"}</p></div>
        ...
        <div className="flex gap-3">
          <button className="rounded-xl border border-slate-300 px-4 py-2 text-sm" type="button" onClick={() => navigate(-1)}>Back</button>
  ...
  return (
    <AppShell title="Registration confirmed">
      <div className="space-y-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        ...
        <p>{registration.participant.participantReference} · Queue {registration.queueNumber} · {registration.registrationStatus}</p>
        <div className="flex flex-wrap gap-3">
...
export function RegistrationHistoryPage() {
  ...
  return <AppShell title="Registration history"><div className="space-y-3">{history.map((item) => <article key={item.id} className="rounded-xl border bg-white p-4"><p className="font-semibold">{item.fromStatus ?? "New"} → {item.toStatus}</p><p className="text-sm text-slate-600">{new Date(item.occurredAt).toLocaleString()} · {item.changedBy?.fullName ?? "Staff"} · {item.reason ?? "No reason"}</p></article>)}</div></AppShell>;
}
...
export function ParticipantHistoryPage() {
  ...
  return <AppShell title="Participant registration history"><div className="space-y-3">{registrations.map((registration) => <article key={registration.id} className="rounded-xl border bg-white p-4"><p className="font-semibold">{registration.event.eventName}</p><p className="text-sm">{registration.registrationStatus} · Queue {registration.queueNumber}</p><Link className="text-sm underline" to={`/registrations/${registration.id}/history`}>Full status history</Link></article>)}</div></AppShell>;
}
