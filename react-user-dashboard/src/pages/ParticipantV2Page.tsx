        ...
        {!searching && results && results.participants.length > 0 ? <>
          ...
          <div className="participant-v2-result-list">
            {results.participants.map((participant) => <article className="participant-v2-result" key={participant.id}>
              ...
              <div className="participant-v2-result-actions"><Link className="secondary" to={`/participants/${participant.id}/edit${eventId ? `?eventId=${encodeURIComponent(eventId)}` : ""}`}><PencilSquareIcon />Edit details</Link>{eventId ? <Link className="primary" to={`/participants/${participant.id}?eventId=${encodeURIComponent(eventId)}`}><ArrowRightIcon />Register</Link> : null}</div>
            </article>)}
          </div>
